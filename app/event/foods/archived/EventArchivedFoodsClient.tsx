...
export default function EventArchivedFoodsClient() {
    ...
    const fetchArchivedFoods = async () => {
      try {
        const response = await fetch(
          '/api/event/foods/archived?teamId=giikuHaku-2025'
        );
        const data = await response.json();

        if (!response.ok) {
          throw new Error(
            data.error || 'アーカイブ食品データの取得に失敗しました'
          );
        }
  ...
  const handleRestoreFood = async (foodId: string) => {
    try {
      const response = await fetch('/api/event/foods/restore', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(result.error || '食品の復元に失敗しました');
      }
