// app/api/actions/restore-food/route.ts (新規作成)
...
export async function POST(req: Request) {
  try {
    ...
    if (existingFoodData?.uid !== uid || existingFoodData?.teamId !== decodedToken.teamId) {
        return NextResponse.json({ error: 'Unauthorized: You do not own this food item or belong to this team.' }, { status: 403 });
    }
    ...
    return NextResponse.json({ message: `Food item ${foodId} restored successfully.` });

  } catch (error: any) {
    console.error('API Error in restore-food:', error);
    return NextResponse.json({ error: error.message || 'Failed to restore food item.' }, { status: 500 });
  }
}