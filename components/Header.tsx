  ...
  return (
    <header className="bg-[#333] py-5 z-20 sticky top-0 w-full">
      <div className="container mx-auto flex justify-between items-center">
        <button onClick={onLogoClick} className="text-xl font-bold cursor-pointer ml-2 text-[#fff]">
          SonaBase
        </button>
        ...
        {isLoggedIn && (
          <nav className="flex items-center">
            {!shouldHideNavLinks && (
              <>
                <Link href="/foods/list" className="mr-4 text-[#fff] hover:text-[#a399ff] text-base sm:text-lg">
                  非常食リスト
                </Link>
                <Link href="/foods/add" className="mr-4 text-[#fff] hover:text-[#a399ff] text-base sm:text-lg">
                  非常食登録
                </Link>
              </>
            )}
